...
export default function ChatPage() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const { success, message, error } = await generateChatResponse(newMessages, selectedModel);
      setMessages(prev => [...prev, { role: "bot", content: success ? (message as string) : `Error: ${error}` }]);
    } catch {
      ...
      <footer className="p-4 border-t border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex-shrink-0 mt-auto">
        <form onSubmit={handleSend} className="max-w-3xl mx-auto flex gap-2 relative">
          <input
            ...
            placeholder={selectedModel ? "Type a message..." : "Waiting for models..."}
