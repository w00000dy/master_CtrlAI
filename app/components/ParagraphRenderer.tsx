...
export const ParagraphRenderer = ({
	...
	return (
		...
		>
			...
			>
				{depth > 0 && (
					<span className={`mt-1.5 leading-none shrink-0 ${isSelected ? "text-blue-400 dark:text-blue-500" : "text-zinc-300 dark:text-zinc-600"}`}>
						↳
					</span>
				)}

				{paragraph.marker && (
					<span className={`shrink-0 mt-0.5 inline-flex items-center justify-center min-w-[2rem] px-2 py-1 text-xs font-bold rounded-md border shadow-sm transition-colors ${isSelected ? "bg-blue-100 dark:bg-blue-800 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-700" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border-zinc-200 dark:border-zinc-700 group-hover:bg-white dark:group-hover:bg-zinc-700"}`}>
						{paragraph.marker}
					</span>
				)}
